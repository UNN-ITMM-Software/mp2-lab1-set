...
TEST(TBitField, clearing_the_bit_twice_leaves_it_in_the_null_state)
{
    const int size = 8;
    TBitField bf(size); //00000000
...
TEST(TBitField, correctness_of_operation_or_on_three_bit_fields)
{
    const int size = 4;
    TBitField bf1(size), bf2(size), bf3(size), result_or(size);
    ...
    EXPECT_EQ(bf1 | bf2 | bf3, result_or);
}

TEST(TBitField, correctness_of_operation_and_on_three_bit_fields)
{
    const int size = 4;
    TBitField bf1(size), bf2(size), bf3(size), result_and(size);
    ...
    EXPECT_EQ(bf1 & bf2 & bf3, result_and);
}