...
{
  bool res = 1;
  if (BitLen != bf.BitLen) res = 0; //Соответственно идет сравнение MemLen и bf.MemLen 
  else
    for (int i = 0; i < MemLen; i++)
...
{
  int TempLen = BitLen;
  if (bf.BitLen > TempLen) TempLen = bf.BitLen;
